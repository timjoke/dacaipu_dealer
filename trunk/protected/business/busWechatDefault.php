...
class busWechatDefault
{
    ...
    public function Proc($dealer_id)
    {
        try
        {
                ...
                case 'event':
                    {
                        ...
                        else if ($event == 'LOCATION')
                        {
                            ...
                            $lng = empty($weixin->msg->Longitude) ? '' : $weixin->msg->Longitude;

                            $busMap = new busBaiduMap();
                            $addr = $busMap->getAddrByGeo($lat, $lng);
                            $reply = $weixin->makeText('您现在在：' . $addr);
                            //$reply = $weixin->makeText('您现在在：' . $lat.','.$lon);
                            $weixin->reply($reply);
                        }
                    }
                    break;
            ...
            //集团用户，让用户选择进入哪家分店
            if ($dealer->dealer_type == DEALER_TYPE_GROUP)
            {
                ...
                $dealers = Dealer::model()->findAllByAttributes(array('dealer_status' => DEALER_STATUS_ONLINE, 'dealer_parent_id' => $dealer->dealer_id,));
                if (isset($dealers))
                {
                    foreach ($dealers as $d)
                    {
                        $pic_url = Pic::model()->getWXPic($d->dealer_id);
                        $banner_url = (!empty($pic_url)) ? busUlitity::get_http_static_url() . $pic_url : busUlitity::get_http_static_url() . 'mobile/img/banner_wx_default.jpg';

                        array_push($news, array(
                            'title' => $d->dealer_name,
                            'description' => '欢迎进入' . $d->dealer_name,
                            'picurl' => $banner_url,
                            'url' => Yii::app()->request->hostInfo . '/wechat/dealer?dealer_id=' . $d->dealer_id . '&wechat_id=' . $fromUser . '&ts=' . $ts));
                    }
                }
            }
            else
